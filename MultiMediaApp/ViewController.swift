...
import UIKit
    ...
    class ViewController: UIViewController {
        ...
        override func viewDidLoad() {
            super.viewDidLoad()
            useTrack(tr: startTrack! )
        }
        
        private func useTrack(tr: Track){
            do {
                    Player = try AVAudioPlayer(contentsOf: URL.init(fileURLWithPath:   Bundle.main.path(forResource:tr.name, ofType: tr.format)!))
 label.text = tr.name
 ...
 Player.play()
        ...
        @IBAction func backButtonAction(_ sender: Any) {
            ...
            if beforeTrackIndex <= 0 {
                ...
                useTrack(tr: trackLast)
            } else {
                useTrack(tr: tracks[beforeTrackIndex])
            }
        }
        ...
        @IBAction func forwardButtonAction(_ sender: Any) {
            ...
            if forwardTrackIndex > tracks.count - 1 {
                useTrack(tr: tracks.first!)
            } else {
                useTrack(tr: tracks[forwardTrackIndex])
            }
        }
